...
for iCluster = 1%:3
    ...
    for iFile = 1:length(FileName)
        ...
        for iState = 1:3
            ...
            TMPCaDataThirdAmp = nan(size(CaDataAll,1),size(TMPEpisodes,2));
            TMPCaDataThirdFre = nan(size(CaDataAll,1),size(TMPEpisodes,2));
            
            for iEpisode = 1:size(TMPEpisodes,2)
                TMPCaDataThirdAmp(:,iEpisode) = nanmean(CaDataAll(:,round(TMPEpisodes(3,iEpisode)*FrameRate)+1:round(TMPEpisodes(3+1,iEpisode)*FrameRate)),2)- ...
                    nanmean(CaDataAll(:,round(TMPEpisodes(1,iEpisode)*FrameRate)+1:round(TMPEpisodes(1+1,iEpisode)*FrameRate)),2);
                TMPCaDataThirdFre(:,iEpisode) = (sum(~isnan(CaDataAll(:,round(TMPEpisodes(3,iEpisode)*FrameRate)+1:round(TMPEpisodes(3+1,iEpisode)*FrameRate))),2)./((TMPEpisodes(2,iEpisode)-TMPEpisodes(1,iEpisode)))) -...
                    (sum(~isnan(CaDataAll(:,round(TMPEpisodes(1,iEpisode)*FrameRate)+1:round(TMPEpisodes(1+1,iEpisode)*FrameRate))),2)./((TMPEpisodes(2,iEpisode)-TMPEpisodes(1,iEpisode))));
            end
    ...
    for iState = 1:3
        Data = [CaDataPerCellState.(BrainStates{iState,1}).Freq, CaDataThirds.(BrainStates{iState,1}).Freq];
        Data(sum(Data,2)==0,:)=[];
        nexttile
        PLOTCorrelations(Data(:,1),Data(:,2),'Spearman',PlotColors{iState,1})
        title(BrainStates{iState,1})
        xlabel({'frequency of Ca^{2+} transients','3^r^d Third - 1^s^t Third (Hz)'})
        ylabel(strcat('Mean frequency of Ca^{2+} transients per epoch (Hz)'))
    ...
    if iCluster == 1
        ...
        set(hFig, 'Position', [0 20 500 500])
        set(gcf,'color','white')
        set(gcf,'name',' ActiveCellsPerEpoch','NumberTitle','off')
    end
    ...
    Data = [];
    for iState = 1:3
        Data(:,iState) = CaDataState.(BrainStates{iState,1}).FreqActCells;
    end
    ...
    Data = [];
    for iState = 1:3
        Data(:,iState) = CaDataState.(BrainStates{iState,1}).AmpActCells*100;
    end
    Data(sum(isnan(Data),2)==3,:) = [];
    Data(isnan(Data))=0;
    
    [eBar,MeanLine] = PlotErrorbar(Data,PlotColors{iCluster,1});
    title('All Active Cells');
    set(gca,'FontSize',8,'TickLength',[0.025 0.025])
    set(gca,'TickDir','out');
    set(gca,"LineWidth",1);
    set(gca, 'FontName', 'Arial')
