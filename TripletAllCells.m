...
for iCluster = 1:3
    ...
    for iFile = 1:length(FileName)
        ...
        for iState = 1:3
            ...
            for iEpisode = 1:size(TMPEpisodes,2)
                if ~isempty(ActiveCells{1,iEpisode})
                    for iThird = 1:3
                        TMPCaDataThirdAmp(iThird,iEpisode) = nanmean(nanmean(CaDataAll(InactiveCells{1,iEpisode},round(TMPEpisodes(iThird,iEpisode)*FrameRate)+1:round(TMPEpisodes(iThird+1,iEpisode)*FrameRate)),2));
                        TMPCaDataThirdFre(iThird,iEpisode) = mean(sum(~isnan(CaDataAll(InactiveCells{1,iEpisode},round(TMPEpisodes(iThird,iEpisode)*FrameRate)+1:round(TMPEpisodes(iThird+1,iEpisode)*FrameRate))),2)./((TMPEpisodes(2,iEpisode)-TMPEpisodes(1,iEpisode))));
                    end
                else
        ...
        for iState = 1:3
            ...
            for iEpisode = 1:size(TMPEpisodes,2)
                if ~isempty(ActiveCells{1,iEpisode})
                    TMPCaDataAmp(1,iEpisode) = nanmean(nanmean(CaDataAll(InactiveCells{1,iEpisode},round(TMPEpisodes(1,iEpisode)*FrameRate)+1:round(TMPEpisodes(4,iEpisode)*FrameRate)),2));
                    TMPCaDataFre(1,iEpisode) = mean(sum(~isnan(CaDataAll(InactiveCells{1,iEpisode},round(TMPEpisodes(1,iEpisode)*FrameRate)+1:round(TMPEpisodes(4,iEpisode)*FrameRate))),2)./(TMPEpisodes(4,iEpisode)-TMPEpisodes(1,iEpisode)));
                    TMPCaDataPerCellAmp = [TMPCaDataPerCellAmp; (nanmean(CaDataAll(InactiveCells{1,iEpisode},round(TMPEpisodes(1,iEpisode)*FrameRate)+1:round(TMPEpisodes(4,iEpisode)*FrameRate)),2))];
                    TMPCaDataPerCellFre = [TMPCaDataPerCellFre; (sum(~isnan(CaDataAll(InactiveCells{1,iEpisode},round(TMPEpisodes(1,iEpisode)*FrameRate)+1:round(TMPEpisodes(4,iEpisode)*FrameRate))),2)./(TMPEpisodes(4,iEpisode)-TMPEpisodes(1,iEpisode)))];
                else
