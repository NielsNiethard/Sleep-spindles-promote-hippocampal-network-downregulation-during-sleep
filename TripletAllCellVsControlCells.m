...
for iState = 1:2
    DataForLMMFreq{iCluster,1} = [DataForLMMFreq{iCluster,1}; [CaData.(BrainStates{iState,1}).Freq(:,1);...
        CaData.(BrainStates{iState,1}).Freq(:,2);...
        CaData.(BrainStates{iState,1}).Freq(:,3)]];
    DataForLMMAmp{iCluster,1} = [DataForLMMAmp{iCluster,1}; CaData.(BrainStates{iState,1}).Amp(:,1);...
        CaData.(BrainStates{iState,1}).Amp(:,2);...
        CaData.(BrainStates{iState,1}).Amp(:,3)];
    StateCond{iCluster,1} = [StateCond{iCluster,1}; repmat(iState,size(CaData.(BrainStates{iState,1}).Freq,1)*3,1)];
    Third{iCluster,1} = [Third{iCluster,1}; ones(size(CaData.(BrainStates{iState,1}).Freq,1),1);...
        ones(size(CaData.(BrainStates{iState,1}).Freq,1),1) *2;...
        ones(size(CaData.(BrainStates{iState,1}).Freq,1),1)*3];
    AnimalName{iCluster,1} = [AnimalName{iCluster,1}; repmat(CaData.(BrainStates{iState,1}).AnimalNameFreq,3,1)];
end
...
for iState = 1:2
    DataForLMMFreq{iCluster,1} = [DataForLMMFreq{iCluster,1}; [CaData.(BrainStates{iState,1}).Amp(:,1);...
        CaData.(BrainStates{iState,1}).Amp(:,2);...
        CaData.(BrainStates{iState,1}).Amp(:,3)]];
    DataForLMMAmp{iCluster,1} = [DataForLMMAmp{iCluster,1}; CaData.(BrainStates{iState,1}).Amp(:,1);...
        CaData.(BrainStates{iState,1}).Amp(:,2);...
        CaData.(BrainStates{iState,1}).Amp(:,3)];
    StateCond{iCluster,1} = [StateCond{iCluster,1}; repmat(iState,size(CaData.(BrainStates{iState,1}).Amp,1)*3,1)];
    Third{iCluster,1} = [Third{iCluster,1}; ones(size(CaData.(BrainStates{iState,1}).Amp,1),1);...
        ones(size(CaData.(BrainStates{iState,1}).Amp,1),1) *2;...
        ones(size(CaData.(BrainStates{iState,1}).Amp,1),1)*3];
    AnimalName{iCluster,1} = [AnimalName{iCluster,1}; repmat(CaData.(BrainStates{iState,1}).AnimalNameAmp,3,1)];
end
